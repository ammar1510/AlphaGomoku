...
class GomokuRenderer:
    ...
    def render_board(self, board):
        ...
        for i in range(self.board_size):
            ...
            end_h = (
                self.board_pixel_size - self.cell_size // 2,
        ...
        for i, row in enumerate(board):
            for j, cell in enumerate(row):
                ...
                if int(cell) == 1:
                    pygame.draw.circle(
                        self.screen, self.black_stone, center, self.cell_size // 2 - 2
                    )
                elif int(cell) == -1:
                    pygame.draw.circle(
                        self.screen, self.white_stone, center, self.cell_size // 2 - 2
                    )
                    pygame.draw.circle(
                        self.screen, self.grid_color, center, self.cell_size // 2 - 2, 1
                    )
        pygame.display.flip()
