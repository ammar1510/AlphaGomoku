...
def init_env(rng, board_size=15, num_boards=1):
    ...
    return {
        ...
        'current_player': jnp.ones((num_boards,), dtype=jnp.int32),  # 1 for black, -1 for white
...
def reset_env(env_state, new_rng=None):
    ...
    rng = new_rng if new_rng is not None else env_state['rng']
...
def step_env(env_state, actions):
    ...
    new_board = jnp.where(
        ...
        current_player[:, jnp.newaxis, jnp.newaxis],  # Shape: (num_envs, 1, 1) -> broadcasts to (num_envs, height, width)
    ...
    current_player_reshaped = new_env_state['current_player'][:, jnp.newaxis, jnp.newaxis]
...
def get_valid_actions(env_state):
    ...
    def get_board_valid_actions(board_idx):
        ...
        padded_indices = jnp.pad(valid_indices, (0, height*width - valid_indices.shape[0]), 
                                constant_values=-1)
...
if __name__ == "__main__":
    ...
    print("-" * 60)
    print(f"{'Board Size':<10} | {'Num Envs':<10} | {'Time (s)':<10} | {'Games/sec':<10} | {'Steps/Game':<10}")
    print("-" * 60)
    
    for board_size in board_sizes:
        for num_envs in num_envs_options:
            # Warmup run to trigger compilation
            warmup_state, warmup_rewards = run_random_episode(board_size, num_envs, seed=0)
            # Force completion of all operations
            warmup_rewards.block_until_ready()
            
            # Timing run
            start_time = time.time()
            final_state, final_rewards = run_random_episode(board_size, num_envs, seed=42)
            ...
            total_steps = jnp.mean(jnp.sum(final_state['board'] != 0, axis=(1, 2))).item() / 2
            games_per_second = num_envs / elapsed
            
            print(f"{board_size:<10} | {num_envs:<10} | {elapsed:<10.3f} | {games_per_second:<10.1f} | {total_steps:<10.1f}")
