...
from flax.training import train_state # Needed for type hint, even if not used directly for state creation
import logging
import hydra
    ...
    while True:
        try:
            ...
            if not (0 <= row < board_size and 0 <= col < board_size):
                print(f"Invalid input: Coordinates must be between 0 and {board_size - 1}.")
...
def get_agent_move(model_apply_fn, params, obs, current_player, valid_mask):
    ...
    masked_logits = jnp.where(flat_valid_mask, pi_dist.logits[0], -jnp.inf) # Index [0] for batch dim
...
def play(cfg: DictConfig):
    ...
    except ValueError: # Handle case where original_cwd is not available (e.g., testing)
        checkpoint_base_dir = cfg.checkpoint_dir
        logging.warning("Could not determine original CWD, assuming checkpoint_dir is absolute or relative to current dir.")
    ...
    logging.info(f"Human plays as {'X (Black)' if human_player == 1 else 'O (White)'}. Agent plays as {'O (White)' if human_player == 1 else 'X (Black)'}.")

    # --- Restore Checkpoint ---
    mngr_options = ocp.CheckpointManagerOptions(create=False) # Don't create if it doesn't exist
    # Use the base checkpoint directory for the manager
    checkpointer = ocp.CheckpointManager(directory=checkpoint_base_dir, options=mngr_options)
    ...
    if target_step not in available_steps:
        logging.error(f"Checkpoint step {target_step} not found in directory: {checkpoint_base_dir}")
    ...
    env_state = jax.tree.map(lambda x: x[0] if x.shape[0] == 1 else x, env_state) # Squeeze batch dim from state
    ...
    while not env_state.dones:
        ...
        valid_mask = np.array(valid_mask_batch[0]) # Squeeze batch dim for human/agent logic
        ...
        if current_player == human_player:
            ...
            player_batch = jnp.expand_dims(env_state.current_players, axis=0) # Already batched in env_state_batch
...
# --- Hydra Main Block ---
@hydra.main(config_path="../../../config", config_name="play", version_base=None) # Adjust config_path
