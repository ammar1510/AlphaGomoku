...
def evaluate_models():
    ...
    if black_params is None or white_params is None:
        logging.error(
            "Couldn't load at least one model checkpoint. Make sure you have trained models."
        )
        return
    ...
    try:
        while pygame_running:
            ...
            while not done and pygame_running:
                ...
                row, col = action[0]
                logging.info(
                    f"Move {move_count + 1}: {player_name} places at ({row}, {col})"
                )
                ...
                if move_count >= board_size * board_size:
                    logging.info(
                        "Maximum number of moves reached. Game ends in a draw."
                    )
                    break
            ...
            logging.info(
                f"Black Wins: {black_wins} ({black_wins/total_games*100:.1f}%)"
            )
            logging.info(
                f"White Wins: {white_wins} ({white_wins/total_games*100:.1f}%)"
            )
