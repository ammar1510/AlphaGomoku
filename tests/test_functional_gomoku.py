...
class TestFunctionalGomoku(unittest.TestCase):
    ...
    def test_init_env(self):
        ...
        # Check dimensions
        self.assertEqual(
            env_state["board"].shape,
            (self.num_boards, self.board_size, self.board_size),
        )
    ...
    def test_step_env(self):
        ...
        # Check if pieces were placed correctly
        self.assertEqual(
            newer_env_state["board"][0, 1, 3], -1
        )  # First board, player -1
        self.assertEqual(
            newer_env_state["board"][1, 2, 4], -1
        )  # Second board, player -1
        self.assertEqual(
            newer_env_state["board"][2, 3, 5], -1
        )  # Third board, player -1
        self.assertEqual(
            newer_env_state["board"][3, 4, 6], -1
